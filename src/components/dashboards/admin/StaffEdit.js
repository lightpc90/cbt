...
const StaffEdit = ({ staff, setStaffs, setStaffData, coursesData }) => {
    ...
    const handleStaffDelete = async () => {
        ...
        try {
            ...
            if (isDeleted.success === false) {
                ...
                setStaffs(prevStaffs => prevStaffs.filter((staff) => (staff._id !== staff._id)))
    ...
    return (
        <div className='max-w-[450px] max-h-[900px] bg-slate-900 py-12 px-5 overflow-auto'>
            <p className='text-center font-bold text-2xl mb-5'>Update Staff Information</p>
            <StaffForm editing={true} staff={staff} setStaffData={setStaffData} coursesData={coursesData} />
            <div className='flex items-center gap-2 mt-5'>
                ...
                <button className='bg-green-800 px-2 py-1 rounded-md hover:border-2 hover:border-white'>
                    Update
                </button>
            </div>
            <div className='mt-10'>
                ...
                {openDeletBox && <div>
                    <label>
                        <p>NB: This action will delete the student entire data. To continue, type the staff ID, {staff.staffID} in the box below</p>
                    ...
                    <div className='space-x-2 mt-3'>
                        <button onClick={handleStaffDelete} className='bg-rose-900 py-1 px-2 hover:bg-rose-700' >{isDeleting ? `Deleting...` : `Delete Staff`}</button>
