...
const page = async ({ params }) => {
  ...
  const studentData = {
    _id: '123', firstname: "Gideon",
    lastname: "Abbey", dept: "Robotics and Mechatronics",
    level: "100", matricNo: "2017/RTM/0923",
    result: [{ code: '', score: '', }]
  }
  ...
  return (
    <div className='h-screen flex items-center justify-center'>
      ...
      <div className='bg-slate-200 text-slate-900 flex flex-col justify-center text-lg p-2 h-[500px] w-[400px] overflow-auto rounded-md shadow-md '>
        ...
        <p className='font-bold'>Name: {`${studentInfo?.firstname} ${studentInfo?.lastname}`}</p>
      ...
      <div className='bg-rose-200 text-white  p-2 h-[500px] w-[400px] overflow-auto rounded-md shadow-md '>
        ...
        {liveCourses.map((liveCourse, i) => (
          <div key={i} className='bg-slate-900 p-3 rounded-md'>
            ...
            <p>Exam date: {liveCourse.question.params.dateAndTime.split('T')[0]} </p>
            <p className='mb-5'>Exam time: {liveCourse.question.params.dateAndTime.split('T')[1]}</p>
