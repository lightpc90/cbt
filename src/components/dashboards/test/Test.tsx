...
const Test = ({ data }) => {
  ...
  function markExam() {
    ...
    Object.entries(answers).forEach(([key, value]) => {
      const match = examQuestions.some(
        (data) => data.question === key && data.answer === value
      );
      if (match) {
        console.log(`Got it! "${key}" = "${value}"`);
        score += 1;
      } else {
        console.log(`Missed it! "${key}" != "${value}"`);
      }
    });
      ...
      <div className="flex flex-col w-10/12 h-full overflow-auto">
        <div className="flex justify-between px-5 py-3 items-center border-4 border-gray-500">
          ...
          <div className="flex h-[70px] justify-end items-center gap-2 ">
            <div className="w-[70px] h-[70px] flex justify-center items-center bg-gray-500 rounded-full overflow-hidden hover:ring-2 hover:ring-rose-700">
              <Image
        ...
        {examQuestions?.length > 0 ? (
          <div className="p-2 w-full">
            ...
            <div className="flex w-full">
              <div className="w-9/12 mx-5 lg:mx-2">
                <QueOptsLayout
              ...
              <div className="flex flex-col justify-between p-2 bg-gray-700 w-3/12 max-h-[500px] lg:w-4/12 lg:h-[320px] overflow-auto rounded-md shadow-md ">
                {/* section holding  the question number selectors */}
                <div className="flex flex-wrap gap-2 max-h-[400px] p-2 overflow-auto bg-slate-900 rounded-lg">
                  {examQuestions?.map(({ question }, i) => (
                    <button
                      ...
                      } ${answers && answers[question] && "bg-gray-800"}`}
                    >
                      {1 + i}
                    </button>
                  ))}
                ...
                <button
                  ...
                  onClick={answerConfirmation}
                  className="bg-slate-900 shadow-md font-semibold hover:bg-rose-800 py-1 rounded-md"
                >
